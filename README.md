# Build a Basic CRUD App with Symfony 4 and React

This example shows how to use Symfony 4 and React to create a basic CRUD app for tracking pun counts in movies, using Okta to handle authentication.
...
```
git clone git@github.com:oktadeveloper/okta-php-symfony-react-crud-example.git
cd okta-php-symfony-react-crud-example
```
...
Edit the Okta configuration in `src/App.js` to fill in your client ID and other application details. (Refer to the (Article Placeholder) for more details.) Then install the dependencies and run the server:
